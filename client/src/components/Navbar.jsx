            ...
            <div className='flex items-center gap-6  relative'>
                <div className='relative'>
                    <button onClick={()=>{
                        ...
                        setShowUserPopup(false)
                    }} className='flex items-center gap-2 bg-white text-gray-600 px-4 py-2 rounded-full text-md hover:bg-[#F3E8FF] transition'>
                        <BsCoin size={20}/>
                        {userData?.credits || 0}
                    </button>

                    {showCreditPopup && (
                        <div className='absolute -right-12.5 mt-3 w-64 bg-white shadow-sm border border-gray-200 rounded-xl p-5 z-50'>
                            <p className='text-sm text-gray-600 mb-4'>Need more credits to continue practicing?</p>
                            <button onClick={()=>navigate("/pricing")} className='w-full bg-gradient-to-r from-[#6600CC] to-[#7A33D1] text-white py-2 rounded-lg text-sm hover:from-[#5B00B3] hover:to-[#7A33D1] transition active:scale-95'>Get more credits</button>
